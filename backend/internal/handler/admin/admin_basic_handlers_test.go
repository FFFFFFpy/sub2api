...
func TestGroupHandlerCreateAllowsExternalOpenAICompatiblePlatform(t *testing.T) {
	router, adminSvc := setupAdminRouter()

	for _, platform := range []string{"external_openai_compatible", "volcengine_coding", "xunfei_coding"} {
		body, _ := json.Marshal(map[string]any{
			"name":                        "group-" + platform,
			"platform":                    platform,
		...
		require.True(t, created.RequestPassthroughEnabled)
	}
}
